...
| 4 | `test_ecmp_random_seq_sparse.py`       | Стратегии выбора Src IP: `random` (псевдослучайные), `sequential` (соседние) и `sparse` (равномерно разреженные) — все три должны балансироваться | \|p − 0.5\| < 0.1 на каждом из путей для каждой из трёх стратегий |
...
| 7 | `test_ecmp_link_failure.py`     | После `to-r2 down` на R1 трафик уходит целиком через R3, без потерь; после `to-r2 up` второй nexthop возвращается и трафик балансируется между R2 и R3   | в FIB остаётся 1 nexthop через R3; \|p − 0.5\| < 0.1 на каждом из путей после восстановления второго nexthop'а |
...
│   ├── test_ecmp_*.py             ← 7 сценариев (13 тестов)
│   └── helpers/
