...
| 4 | `test_ecmp_random_seq.py`       | Стратегии выбора Src IP: `random` (псевдослучайные), `sequential` (соседние) и `sparse` (равномерно разреженные) — все три должны балансироваться | \|p − 0.5\| < 0.1 на каждом из путей для каждой из трёх стратегий |
